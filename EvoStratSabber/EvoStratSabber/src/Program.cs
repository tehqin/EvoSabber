...
namespace EvoStratSabber
{
   class Program
   {
      static void Main(string[] args)
      {
			...
			Console.WriteLine("Search decks for class: "+className);
         CardClass heroClass = GetClassFromName(className);
         List<Card> choices = GetCards(heroClass);
         var search = new EvolutionaryStrategies(heroClass, choices); 
         search.Run();
      }
